...
export default function LiveTracking() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-20">
      <div>
        <h1 className="text-3xl font-bold text-[var(--on-surface)] tracking-tight flex items-center gap-2" style={{ fontFamily: "var(--font-heading)" }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-[var(--surface-container-lowest)] border border-[var(--outline-variant)]/20 rounded-sm shadow-sm p-8">
          <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
            <h3 className="font-bold text-[var(--on-surface)] text-lg" style={{ fontFamily: "var(--font-heading)" }}>Yard digital twin</h3>
          ...
          <div className="relative mb-6">
            ...
            <input
              ...
              className="w-full bg-[var(--surface-container-low)] border-none text-[var(--on-surface)] placeholder:text-[var(--outline)]/70 rounded-xl py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
            />
          </div>
          <div className="flex flex-wrap gap-3">
            {(yard.spots || []).map((spot: any) => {
              ...
              const bg =
                status === "breach" ? "bg-red-500/10 border-red-500/40 text-red-700" :
                status === "warning" ? "bg-amber-500/10 border-amber-500/40 text-amber-800" :
                status === "normal" ? "bg-teal-500/10 border-teal-500/40 text-teal-700" :
                "bg-[var(--surface-container-low)] border-[var(--outline-variant)]/30 text-[var(--on-surface-variant)]";
        ...
        <div className="bg-[var(--surface-container-lowest)] border border-[var(--outline-variant)]/20 rounded-sm shadow-sm p-8">
          <h3 className="font-bold text-[var(--on-surface)] text-lg mb-6 flex items-center gap-2" style={{ fontFamily: "var(--font-heading)" }}>
          ...
          <div className="space-y-4 max-h-[480px] overflow-y-auto pr-1">
            ...
            {logs.map((l) => (
              <div key={l.id} className="flex items-start gap-3">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${l.event_type === "denied" ? "bg-red-500/15 text-red-700" : l.event_type === "exit" ? "bg-[var(--surface-container-high)] text-[var(--on-surface-variant)]" : "bg-teal-500/15 text-teal-700"}`}>
        ...
        {selected && (
          ...
          >
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center justify-between py-1.5">
                ...
                <select
                  ...
                  className="bg-[var(--surface-container-low)] border border-[var(--outline-variant)] text-[var(--on-surface)] rounded-lg px-2 py-1 text-xs font-semibold"
                >
            ...
            <div className="mt-6 pt-4 border-t border-slate-800">
              ...
              {selected.hazmat_class && <p className="text-xs text-amber-800 bg-amber-500/10 border border-amber-500/30 rounded-lg px-2.5 py-1.5 mb-2 inline-block font-semibold">Hazmat class {selected.hazmat_class}</p>}
              {Array.isArray(selected.damage_photos) && selected.damage_photos.length > 0 && (
                <div className="space-y-1.5 mb-3">
                  {selected.damage_photos.map((d: any, i: number) => (
                    <div key={i} className="text-xs bg-red-500/10 border border-red-500/30 text-red-700 rounded-lg px-2.5 py-1.5">
              ...
              <form onSubmit={submitInspection} className="space-y-2">
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    ...
                    <input value={inspectionForm.hazmat_class} onChange={(e) => setInspectionForm({ ...inspectionForm, hazmat_class: e.target.value })} placeholder="e.g. 3" className="w-full bg-[var(--surface-container-low)] border border-[var(--outline-variant)] text-[var(--on-surface)] rounded-lg px-2.5 py-1.5 text-xs" />
                  </div>
                  <div className="space-y-1">
                    ...
                    <input type="number" value={inspectionForm.tare_weight_kg} onChange={(e) => setInspectionForm({ ...inspectionForm, tare_weight_kg: e.target.value })} className="w-full bg-[var(--surface-container-low)] border border-[var(--outline-variant)] text-[var(--on-surface)] rounded-lg px-2.5 py-1.5 text-xs" />
                  </div>
                </div>
                <div className="flex items-end gap-2">
                  <div className="flex-1 space-y-1">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Report damage</label>
                    <input value={inspectionForm.damage_note} onChange={(e) => setInspectionForm({ ...inspectionForm, damage_note: e.target.value })} placeholder="Describe any damage found" className="w-full bg-[var(--surface-container-low)] border border-[var(--outline-variant)] text-[var(--on-surface)] rounded-lg px-2.5 py-1.5 text-xs" />
                  </div>
                  <button type="submit" disabled={inspectionBusy} className="bg-[var(--primary)] text-white px-3 py-1.5 rounded-sm text-xs font-bold hover:bg-[var(--primary-container)] disabled:opacity-50 flex items-center gap-1.5">
            ...
            {selected.equipment_type === "reefer" && (
              <div className="mt-6 pt-4 border-t border-slate-800">
                ...
                {reeferReadings[0] ? (
                  <div className={`rounded-xl px-3 py-2.5 mb-3 text-xs font-semibold border ${reeferReadings[0].status === "critical" ? "bg-red-500/10 border-red-500/30 text-red-700" : reeferReadings[0].status === "warning" ? "bg-amber-500/10 border-amber-500/30 text-amber-800" : "bg-teal-500/10 border-teal-500/30 text-teal-700"}`}>
                ...
                <form onSubmit={submitReeferReading} className="flex items-end gap-2">
                  <div className="flex-1 space-y-1">
                    ...
                    <input required type="number" step="0.1" value={reeferForm.temperature_c} onChange={(e) => setReeferForm({ ...reeferForm, temperature_c: e.target.value })} className="w-full bg-[var(--surface-container-low)] border border-[var(--outline-variant)] text-[var(--on-surface)] rounded-lg px-2.5 py-1.5 text-xs" />
                  </div>
                  <div className="flex-1 space-y-1">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Fuel %</label>
                    <input type="number" min="0" max="100" value={reeferForm.fuel_level_pct} onChange={(e) => setReeferForm({ ...reeferForm, fuel_level_pct: e.target.value })} className="w-full bg-[var(--surface-container-low)] border border-[var(--outline-variant)] text-[var(--on-surface)] rounded-lg px-2.5 py-1.5 text-xs" />
                  </div>
            ...
            {user?.role === "superadmin" && (
              <div className="mt-6 pt-4 border-t border-slate-800">
                ...
                {!transferOpen ? (
                  <button type="button" onClick={() => setTransferOpen(true)} className="w-full text-xs font-bold bg-[var(--surface-container-high)] text-[var(--on-surface-variant)] rounded-sm py-2 hover:bg-[var(--surface-container-highest)] transition-all">
                    Transfer to another facility
                  </button>
                ) : (
                  <form onSubmit={submitTransfer} className="space-y-2.5">
                    <select required value={transferForm.destination_facility_id} onChange={(e) => setTransferForm({ ...transferForm, destination_facility_id: e.target.value })} className="w-full bg-[var(--surface-container-low)] border border-[var(--outline-variant)] text-[var(--on-surface)] rounded-lg px-2.5 py-1.5 text-xs">
                      <option value="">Destination facility...</option>
                      {facilities.map((f: any) => (
                        <option key={f.id} value={f.id}>{f.name}</option>
                      ))}
                    </select>
                    <input type="datetime-local" value={transferForm.eta} onChange={(e) => setTransferForm({ ...transferForm, eta: e.target.value })} className="w-full bg-[var(--surface-container-low)] border border-[var(--outline-variant)] text-[var(--on-surface)] rounded-lg px-2.5 py-1.5 text-xs" placeholder="ETA" />
                    <input value={transferForm.notes} onChange={(e) => setTransferForm({ ...transferForm, notes: e.target.value })} placeholder="Notes (optional)" className="w-full bg-[var(--surface-container-low)] border border-[var(--outline-variant)] text-[var(--on-surface)] rounded-lg px-2.5 py-1.5 text-xs" />
                    <div className="flex gap-2">
                      <button type="button" onClick={() => setTransferOpen(false)} className="flex-1 text-xs font-bold bg-[var(--surface-container-high)] text-[var(--on-surface-variant)] rounded-sm py-1.5 hover:bg-[var(--surface-container-highest)]">Cancel</button>
              ...
              <div className="space-y-3">
                {timeline.map((ev, i) => (
                  <div key={i} className="flex items-start gap-2.5 text-xs">
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-[var(--on-surface)]">{ev.label}</p>
...
function KPI({ icon, label, value, color }: any) {
  const colors: any = {
    indigo: "bg-[var(--surface-container-high)] text-[var(--on-surface)]",
    teal: "bg-[var(--secondary-container)] text-[var(--on-secondary-container)]",
    amber: "bg-[var(--tertiary-fixed)] text-[var(--on-tertiary-fixed-variant)]",
    red: "bg-[var(--error-container)] text-[var(--on-error-container)]",
  };
  return (
    <div className="bg-[var(--surface-container-lowest)] border border-[var(--outline-variant)]/20 rounded-sm p-5 flex items-center gap-3 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
      <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${colors[color]}`}>{icon}</div>
      <div>
        <p className="text-xl font-bold text-[var(--on-surface)] leading-none" style={{ fontFamily: "var(--font-heading)" }}>{value}</p>
...
function Row({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex justify-between items-center py-1.5 border-b border-slate-800/60">
      ...
      <span className={`font-bold text-[var(--on-surface)] ${mono ? "font-mono" : ""}`}>{value}</span>
    </div>
  );
}
